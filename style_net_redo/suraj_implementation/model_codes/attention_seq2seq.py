...
class GenreLSTM(object):
    ...
    def prepare_unidiretional(self, glorot=True):
        ...
        with tf.variable_scope("jencode") as scope:



            self.jazz_encoder_cell = tf.contrib.rnn.LSTMCell(self.output_size, forget_bias=1.0)
            self.jazz_encoder_cell = tf.contrib.rnn.DropoutWrapper(self.jazz_encoder_cell, input_keep_prob=self.input_keep_prob, output_keep_prob=self.output_keep_prob)
            self.jazz_encoder_outputs, self.jazz_encoder_states = tf.nn.dynamic_rnn(self.jazz_encoder_cell, self.enc_outputs, sequence_length=self.seq_len, dtype=tf.float32)

            # Create an attention mechanism
            self.jazz_attention_mechanism = tf.contrib.seq2seq.LuongAttention(
                self.output_size, self.jazz_encoder_outputs,
                memory_sequence_length=self.seq_len)


            self.jazz_decoder_cell = tf.contrib.rnn.LSTMCell(self.output_size, forget_bias=1.0)
            self.jazz_decoder_cell = tf.contrib.rnn.DropoutWrapper(self.jazz_decoder_cell, input_keep_prob=self.input_keep_prob, output_keep_prob=self.output_keep_prob)

            #CREATE DECODER CELL
            self.jazz_decoder_cell = tf.contrib.seq2seq.AttentionWrapper(
                                        self.jazz_decoder_cell, self.jazz_attention_mechanism,
                                        attention_layer_size=25)

            self.jazz_decoder_helper = tf.contrib.seq2seq.TrainingHelper(self.jazz_encoder_outputs,self.seq_len)
            self.jazz_decoder_process =tf.contrib.seq2seq.BasicDecoder(self.jazz_decoder_cell,self.jazz_decoder_helper,self.jazz_encoder_states)
            self.jazz_outputs= tf.contrib.seq2seq.dynamic_decode(self.jazz_decoder_process)

            # import pdb
            # pdb.set_trace()
            self.jazz_linear_out = self.jazz_outputs[0].rnn_output
        ...
        with tf.variable_scope("cencode") as scope:

            self.classical_encoder_cell = tf.contrib.rnn.LSTMCell(self.output_size, forget_bias=1.0)
            self.classical_encoder_cell = tf.contrib.rnn.DropoutWrapper(self.classical_encoder_cell,
                                                                  input_keep_prob=self.input_keep_prob,
                                                                  output_keep_prob=self.output_keep_prob)
            self.classical_encoder_outputs, self.classical_encoder_states = tf.nn.dynamic_rnn(self.classical_encoder_cell,
                                                                                    self.enc_outputs,
                                                                                    sequence_length=self.seq_len,
                                                                                    dtype=tf.float32)
            self.classical_attention_mechanism = tf.contrib.seq2seq.LuongAttention(
                self.output_size, self.classical_encoder_outputs,
                memory_sequence_length=self.seq_len)

            self.classical_decoder_cell = tf.contrib.rnn.LSTMCell(self.output_size, forget_bias=1.0)
            self.classical_decoder_cell = tf.contrib.rnn.DropoutWrapper(self.classical_decoder_cell,
                                                                   input_keep_prob=self.input_keep_prob,
                                                                   output_keep_prob=self.output_keep_prob)
            self.classical_decoder_cell = tf.contrib.seq2seq.AttentionWrapper(
                                        self.classical_decoder_cell, self.classical_attention_mechanism,
                                        attention_layer_size=25)
            

            self.classical_projection_layer = None
            self.classical_decoder_helper = tf.contrib.seq2seq.TrainingHelper(self.classical_encoder_outputs, self.seq_len)
            self.classical_decoder_process = tf.contrib.seq2seq.BasicDecoder(self.classical_decoder_cell,
                                                                        self.classical_decoder_helper,
                                                                        self.classical_encoder_states)
            self.classical_outputs = tf.contrib.seq2seq.dynamic_decode(self.classical_decoder_process)

            self.classical_linear_out = self.classical_outputs[0].rnn_output
